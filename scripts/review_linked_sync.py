...
def simulate(
    ...
    *,
    ...
    result = SimResult(planner, effect, true_rate)
    ...
    def restart_strip(at: float) -> None:
        ...
        reference.parse(ideal, int(round(at * 1000.0)))
    ...
    def write_dot(at: float, reason: str) -> None:
        ...
        if planner == "new":
            ...
            timed = apply_device_timing(
                ideal,
                DeviceTiming(anchor=state.pro_anchor, rate=rate),
                led_count=2,
                now=at,
    ...
    for index in range(1, frames):
        ...
        if lap and current.lap_dot and current.lap_real:
            content = current.phase_ms + (dot_clock(t) - current.parse_dot_ms) * current.lap_real / current.lap_dot
