...
struct UsageHistoryTests {
    ...
    func decodes() throws {
        ...
        #expect(!pricing.approximate, "a table-priced quote is a list price, not an approximation")
        #expect(pricing.currency == "USD")
        #expect(pricing.asOf == "2026-09-20")
        #expect(pricing.tableVersion == "jrbar-rates-v3")
        #expect(pricing.model == "sonnet")
        #expect(pricing.source == "table")
        #expect(!pricing.estimated)
        #expect(history.costsApproximate, "the one estimated record still hedges the totals")
        #expect(history.estimatedRecords == 1)
        #expect(history.unpricedRecords == 0)
        #expect(history.unpricedModels.isEmpty)
