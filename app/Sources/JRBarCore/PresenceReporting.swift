...
/// The Mac's Focus rides the same report (`focus`, the app's own
/// `INFocusStatusCenter` reading): the daemon's own helper cannot read
/// Focus, so without it Follow Focus has nothing to follow.
///
/// A report stands for 180 s, so the rules are few: an edge goes out at
/// once; while a sensor is live or a Focus is on the same report is
/// renewed every minute, well inside that window; the end goes out once
/// and then nothing does — a quiet report left to go stale still reads as
/// no call and no Focus. The first report of a connection always goes,
/// quiet or not, so a call an earlier run reported ends now instead of
/// three minutes from now. Pure, so the timing is pinned without a socket.
public struct PresenceReporting: Equatable, Sendable {
    ...
    /// The report a reading makes: the two sensors the app watches, and
    /// the Focus when the app can read it (nil leaves the key out, and the
    /// daemon on its own reading). No screen share, lock or idle — those
    /// stay the daemon's until the app has one to give.
    public static func report(for reading: NotchSensorState, focus: Bool? = nil) -> CorePresenceReport {
        CorePresenceReport(mic: reading.microphoneInUse, camera: reading.cameraInUse, focus: focus)
    }

    /// A report the daemon must keep hearing: a call, or a Focus that is
    /// on. Anything else is said once and left to go stale.
    public static func renews(_ report: CorePresenceReport) -> Bool {
        report.sensingCall || report.focus == true
    }

    /// What to send now, if anything. Nothing while the daemon is away.
    public func due(reading: NotchSensorState, focus: Bool? = nil, connected: Bool, now: Date) -> CorePresenceReport? {
        guard connected else { return nil }
        let report = Self.report(for: reading, focus: focus)
        ...
        if Self.renews(report), now.timeIntervalSince(lastSentAt) >= Self.renewInterval { return report }
        return nil
    }

    /// When `due` next owes something for an unchanged reading: the retry
    /// after a failure, else the renewal while a call or a Focus is on;
    /// nil while nothing is owed (an edge asks on its own).
    public func nextCheck(connected: Bool) -> Date? {
        ...
        guard let lastSent, Self.renews(lastSent), let lastSentAt else { return nil }
