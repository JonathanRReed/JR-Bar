...
| F | Pi + Gemini providers, Dot identity, linked Pro+Dot (Dot at `linked_dot_scale` of the strip), auto-dim, Control Center in the daemon, packaged app with bundled daemon + shim, Sparkle with a fresh key, forecast + pricing, quit turns strips off | done 2026-09-10 (not notarized: no `jrbar-notary` profile yet; Creator Micro 2 verified pad-off only; macOS prefs daemon on this Mac is wedged until a reboot) |
