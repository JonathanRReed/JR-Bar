...
| Movement anchor (fold starts wherever the lid rested) | no | no | yes (auto-anchor when still) | Done | The default (every older file moved to it; the set angle is kept for "Set angle"). `FoldAnchor.movement` + `MoveAnchor`: still ≥400 ms while flat re-seats; a reopen that stops short (up >3° from the low, still 1 s) re-seats; arms on 3° of travel down in both looks; delta 0 above the anchor; dwell re-seats |
