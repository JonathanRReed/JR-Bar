...
| Movement anchor (fold starts wherever the lid rested) | no | no | yes (auto-anchor when still) | Done | The default (every older file moved to it; the set angle is kept for "Set angle"). `FoldAnchor.movement` + `MoveAnchor`: still ≥400 ms while flat re-seats; arms on first move (3° in the Duo); delta 0 above the anchor; dwell re-seats |
...
| Black across a full close, unfold from black | n/a | n/a | n/a (macTilt draws over the lock screen) | Done | Duo `FoldBlackout`: a flat black hold with capture stopped, 3 s watchdog, drops on sleep/lock/session; never above the lock screen, no SkyLight |
...
| Screen-capture indicator only while folding | yes (Screen Recording, on-device only) | n/a (minimal capture) | yes (privacy: frames in GPU only) | Done | Streams live only from the first real move off the resting angle (3° in the Duo) or inside `FoldArming`'s band for a set angle (activation + 12°), with a 2 s linger |
