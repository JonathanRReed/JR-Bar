...
| Synced lyrics | yes | Done (LRCLIB) | `/api/get` → `/api/search` fallback — free, keyless, the same source boring.notch/Atoll/Notchy use; LRC-parsed, cached per track, playhead-synced quiet line on the media card |
