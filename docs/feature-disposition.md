...
| --- | ---: | --- | --- |
| T3 Code query-only usage compatibility | 5 | Keep | `src/jrbar/usage_graph_worker.py` scans T3's local SQLite without mutation or credentials. |
...
| `jrbar serve` control/observability API | 4 | Keep | The loopback schema-v2 API remains a privacy-redacted surface for Stream Deck and scripts. It is useful even without first-party orchestration. |
| Creator Micro 2 / Control Center controls | 5 | Implemented in source, release-gated | Saved app-switching and shortcut mappings, native controls, single-owner input/output, shared colours/dimming, and revocable delivery are wired. Device-layer onboarding, a rebuilt installed app, and physical input/output verification remain. The external Agent Deck snapshot compatibility was removed in 0.8. |
