...
| `devices.N.led_direction` | `forward` \| `reversed` | `forward` | Which way round the strip is mounted. Reversed mirrors every agent light drawn for that device (LED `i` becomes `n-1-i`, rolls turn the other way). The Screen Bar ignores it, and so does a linked Dot whose role is not `status` (it plays the Pro's program, narrowed). |
