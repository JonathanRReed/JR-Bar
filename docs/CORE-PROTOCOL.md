...
| `devices.N.led_direction` | `forward` \| `reversed` | `forward` | Which way round the strip is mounted. Reversed mirrors every agent light drawn for that device (LED `i` becomes `n-1-i`, rolls turn the other way), and a `preview_program` played on it. The Screen Bar ignores it. A linked Dot whose role is not `status` ignores its own setting and follows the Pro in desk order: the daemon turns a reversed Pro's program back before narrowing it, so flipping the Pro never turns the Dot's light round. |
