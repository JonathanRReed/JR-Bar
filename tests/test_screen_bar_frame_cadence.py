...
def test_the_pipeline_runs_at_the_policy_rate_not_the_callback_rate__and_1_more(monkeypatch) -> None:
    # --- scenario: the_pipeline_runs_at_the_policy_rate_not_the_callback_rate
    for callback_hz, policy_fps, expected in [
    ...
    # --- scenario: a_callback_with_no_schedule_still_presents_every_tick
    monkeypatch.undo()
...
def test_a_frame_with_no_sampler_output_does_not_spend_the_interval__and_1_more(monkeypatch,) -> None:
    # --- scenario: a_frame_with_no_sampler_output_does_not_spend_the_interval
    ...
    # --- scenario: the_gate_still_rate_limits_once_real_frames_are_flowing
    monkeypatch.undo()
...
def test_main_thread_jitter_does_not_drop_frames_at_the_driver_rate__and_1_more(monkeypatch) -> None:
    # --- scenario: main_thread_jitter_does_not_drop_frames_at_the_driver_rate
    ...
    """
    for jitter_ms in [1.0, 2.0, 4.0, 8.0]:
    ...
    # --- scenario: jitter_does_not_break_the_two_to_one_case_either
    monkeypatch.undo()
    device, clock, virtual_device = _device(monkeypatch, 30.0, driver_fps=60.0)
    frames = _drive(
        device,
...
def test_the_hold_is_half_a_driver_period_short_of_the_interval__and_1_more() -> None:
    # --- scenario: the_hold_is_half_a_driver_period_short_of_the_interval
    ...
    # --- scenario: the_policy_never_names_a_rate_its_driver_cannot_produce
...
def test_every_reachable_schedule_delivers_exactly_what_it_names(monkeypatch) -> None:
    ...
    for thermal, low_power, refresh in [
        ("nominal", False, 120.0),
        ("fair", False, 120.0),
        ("serious", False, 60.0),
...
def test_a_cadence_change_presents_on_the_very_next_callback__and_2_more(monkeypatch) -> None:
    # --- scenario: a_cadence_change_presents_on_the_very_next_callback
    ...
    # --- scenario: an_unchanged_cadence_does_not_reset_the_gate
    monkeypatch.undo()
    ...
    # --- scenario: a_new_program_presents_on_the_very_next_callback
    monkeypatch.undo()
