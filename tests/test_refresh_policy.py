...
def test_menu_open_selects_stale_and_missing_but_not_fresh_providers__and_2_more() -> None:
    # --- scenario: menu_open_selects_stale_and_missing_but_not_fresh_providers
    states = (
    ...
    # --- scenario: menu_open_excludes_ineligible_in_flight_and_backed_off_providers
    states = (
    ...
    # --- scenario: exact_freshness_boundary_is_due
...
def test_low_power_extends_freshness_but_never_blocks_first_load__and_2_more() -> None:
    # --- scenario: low_power_extends_freshness_but_never_blocks_first_load
    states = (
    ...
    # --- scenario: implausibly_future_success_is_treated_as_missing
    plan = plan_menu_open_refresh(
    ...
    # --- scenario: nonfinite_success_timestamp_cannot_stay_fresh
    plan = plan_menu_open_refresh(
...
def test_retry_delay_doubles_and_caps_without_jitter__and_2_more() -> None:
    # --- scenario: retry_delay_doubles_and_caps_without_jitter
    ...
    # --- scenario: plan_preserves_input_order_and_emits_duplicate_provider_once
    first = source("claude")
    second = source("codex")
    third = source("gemini")
    plan = plan_menu_open_refresh(
    ...
    # --- scenario: failure_and_success_transitions_are_immutable_and_reset_backoff
...
def test_provider_id_property_is_read_only_compatibility_projection__and_2_more() -> None:
    # --- scenario: provider_id_property_is_read_only_compatibility_projection
    ...
    # --- scenario: registry_capabilities_are_enumerated_without_collapsing_provider_rows
    ...
    assert len(plan.invocations) == len(rows) == 22

    # --- scenario: same_provider_different_sources_remain_independently_due
...
def test_nonfinite_planning_time_schedules_nothing__and_2_more() -> None:
    # --- scenario: nonfinite_planning_time_schedules_nothing
    ...
    # --- scenario: nonfinite_retry_boundary_fails_closed
    ...
    # --- scenario: boundary_attempt_retention_is_deterministic_and_capped_at_64
...
def test_boundary_attempt_retention_deduplicates_and_normalizes_set_order__and_2_more() -> None:
    # --- scenario: boundary_attempt_retention_deduplicates_and_normalizes_set_order
    retained = retain_attempted_boundary_keys(
    ...
    # --- scenario: failure_retry_schedule_is_an_explicit_exponential_boundary
    ...
    # --- scenario: retry_after_zero_is_not_confused_with_an_absent_boundary
    schedule = resolve_retry_schedule(
        completed_at=100.0,
...
def test_retry_after_is_clamped_to_a_bounded_one_hour_horizon__and_2_more() -> None:
    # --- scenario: retry_after_is_clamped_to_a_bounded_one_hour_horizon
    schedule = resolve_retry_schedule(
        completed_at=100.0,
        consecutive_failures=1,
        retry_at=100_000.0,
    ...
    # --- scenario: retry_schedule_rejects_invalid_completion_time
    ...
    # --- scenario: retry_schedule_rejects_invalid_explicit_boundary
    for value in (float("nan"), float("inf"), -1.0):
        with pytest.raises(ValueError):
            resolve_retry_schedule(
                completed_at=100.0,
