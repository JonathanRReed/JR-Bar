...
def test_failure_signal_plays_exactly_two_repetitions_then_restores__and_2_more() -> None:
    # --- scenario: failure_signal_plays_exactly_two_repetitions_then_restores
    ...
    # --- scenario: warm_state_watermark_does_not_replay_restored_failure
    coordinator = FiniteSignalCoordinator(failure_cycle_seconds=1.0)
    ...
    # --- scenario: duplicate_observation_never_extends_active_deadline
    coordinator = FiniteSignalCoordinator(failure_cycle_seconds=1.0)
...
def test_consumed_keys_are_bounded_by_count_and_age__and_2_more() -> None:
    # --- scenario: consumed_keys_are_bounded_by_count_and_age
    ...
    # --- scenario: many_failures_coalesce_to_one_pending_cue_and_cannot_extend_burst
    coordinator = FiniteSignalCoordinator(
    ...
    # --- scenario: actionable_attention_preempts_failure_without_replay_after_resolution
    coordinator = FiniteSignalCoordinator(failure_cycle_seconds=1.0)
...
def test_unchanged_visible_failure_does_not_replay_after_consumed_ttl_expires__and_2_more() -> None:
    # --- scenario: unchanged_visible_failure_does_not_replay_after_consumed_ttl_expires
    coordinator = FiniteSignalCoordinator(
        failure_cycle_seconds=1.0,
    ...
    # --- scenario: legacy_coordinator_rejects_nonexact_or_over_budget_repetitions
    for repetitions in (True, False, 0, -1, 3, 1.5, "2"):
        coordinator = FiniteSignalCoordinator(failure_cycle_seconds=1.0)
    ...
    # --- scenario: legacy_coordinator_rejects_invalid_or_wall_clock_time_without_mutation
    for invalid_now in (math.nan, math.inf, -1.0, 1_700_000_000.0):
        coordinator = FiniteSignalCoordinator(failure_cycle_seconds=1.0)
...
def test_finite_cues_have_one_active_and_one_distinct_pending__and_2_more() -> None:
    # --- scenario: finite_cues_have_one_active_and_one_distinct_pending
    ...
    # --- scenario: duplicate_cue_does_not_queue_or_extend_the_active_deadline
    coordinator = FiniteCueCoordinator()
    ...
    # --- scenario: higher_priority_new_cue_replaces_pending_without_extending_active
    coordinator = FiniteCueCoordinator()
...
def test_three_distinct_cues_coalesce_to_one_static_overflow_summary__and_2_more() -> None:
    # --- scenario: three_distinct_cues_coalesce_to_one_static_overflow_summary
    coordinator = FiniteCueCoordinator()
    ...
    # --- scenario: invalid_or_over_budget_cues_never_enter_motion_state
    coordinator = FiniteCueCoordinator()
    invalid = (
        cue("", repetitions=1),
    ...
    # --- scenario: consumed_watermark_evicts_to_exactly_256_keys
    coordinator = FiniteCueCoordinator()
...
def test_repeated_257_cue_observation_does_not_replay_trimmed_visible_episode__and_2_more() -> None:
    # --- scenario: repeated_257_cue_observation_does_not_replay_trimmed_visible_episode
    coordinator = FiniteCueCoordinator()
    ...
    # --- scenario: repeated_257_cue_watermark_does_not_replay_trimmed_visible_episode
    coordinator = FiniteCueCoordinator()
    visible = tuple(cue(f"episode-{index}", repetitions=1) for index in range(257))
    ...
    # --- scenario: overflow_replacement_is_fresh_without_replaying_retained_episodes
    coordinator = FiniteCueCoordinator()
    visible = tuple(cue(f"episode-{index}", repetitions=1) for index in range(257))
...
def test_trimmed_overflow_episode_requires_disappearance_before_reappearance__and_2_more() -> None:
    # --- scenario: trimmed_overflow_episode_requires_disappearance_before_reappearance
    coordinator = FiniteCueCoordinator()
    visible = tuple(cue(f"episode-{index}", repetitions=1) for index in range(257))
    coordinator.establish_watermark(visible, now=10.0)
    ...
    # --- scenario: non_event_refreshes_cannot_replay_the_same_upstream_episode_key
    for refresh_reason in (
        "reconnect",
        "ordinary_refresh",
    ...
    # --- scenario: warm_restore_consumes_upstream_keys_without_motion_or_replay
    coordinator = FiniteCueCoordinator()
...
def test_sleep_consumes_active_pending_and_new_cues_without_wake_replay__and_2_more() -> None:
    # --- scenario: sleep_consumes_active_pending_and_new_cues_without_wake_replay
    coordinator = FiniteCueCoordinator()
    first = cue("failure-1")
    ...
    # --- scenario: late_advance_does_not_replay_pending_cue_after_its_episode_window
    coordinator = FiniteCueCoordinator()
    coordinator.observe(
        (
    ...
    # --- scenario: regressing_or_wall_clock_time_cannot_mutate_cue_state
    coordinator = FiniteCueCoordinator()
...
def test_invalid_play_motion_flag_does_not_advance_the_monotonic_watermark__and_1_more() -> None:
    # --- scenario: invalid_play_motion_flag_does_not_advance_the_monotonic_watermark
    coordinator = FiniteCueCoordinator()
    first = coordinator.observe(
        (cue("failure-1", repetitions=1),),
        now=10.0,
    ...
    # --- scenario: budget_configuration_cannot_weaken_hard_cue_bounds
    coordinator = FiniteCueCoordinator(
        FiniteCueBudget(
            max_repetitions=99,
            max_active=99,
