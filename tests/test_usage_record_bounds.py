...
def test_oversized_record_is_partial_and_following_record_survives(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_recursive_json_is_partial_and_following_record_survives(tmp_path: Path) -> None:
    for provider in ("claude", "codex"):
        path = tmp_path / f"{provider}.jsonl"
