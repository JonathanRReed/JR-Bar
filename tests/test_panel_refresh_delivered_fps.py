...
def test_the_fallback_timer_delivers_the_same_rate_on_every_panel(monkeypatch) -> None:
    ...
    for panel in ALL_PANELS:
        for label, environment, gentle, expected in TIMER_TABLE:
            ...
            assert schedule.driver is RenderDriverKind.TIMER, (panel, label)
            assert schedule.driver_fps == render_policy.TIMER_DRIVER_FPS
            assert schedule.cadence.fps == pytest.approx(expected), (panel, label)
            assert _delivered_fps(monkeypatch, schedule) == pytest.approx(
                expected, abs=0.2
            ), (panel, label)
...
def test_the_display_link_delivers_a_whole_fraction_of_what_it_negotiated(
    ...
    for panel, driver_fps, transition_fps, breathe_fps in DISPLAY_LINK_TABLE:
        for gentle, expected in ((False, transition_fps), (True, breathe_fps)):
            ...
            assert schedule.driver is RenderDriverKind.DISPLAY_LINK, panel
            assert schedule.driver_fps == pytest.approx(driver_fps), panel
...
def test_no_panel_costs_a_frame_the_driver_could_have_delivered() -> None:
    ...
    for panel in ALL_PANELS:
        for link in (True, False):
            for label, environment, gentle, _expected in TIMER_TABLE:
                ceiling = choose_render_cadence(
                    environment, True, gentle_motion=gentle
                ).fps
                schedule = choose_render_schedule(
                    environment,
                    True,
                ...
                assert schedule.cadence.fps == pytest.approx(best), (
                    f"{panel:g} Hz / {label}: driver runs at {schedule.driver_fps:g} "
                    f"and could have delivered {best:g} under a {ceiling:g} ceiling, "
                    f"but the policy named {schedule.cadence.fps:g}"
                )
                ...
                assert ratio == pytest.approx(round(ratio)), (
                    ...
                    f"fraction of the {schedule.driver_fps:g} Hz driver, so the gate "
                    f"has to drop frames unevenly to produce it"
                )
