...
def test_display_services_read_is_authoritative_for_active_display__and_2_more() -> None:
    # --- scenario: display_services_read_is_authoritative_for_active_display
    with patch.object(
        display_brightness,
    ...
    # --- scenario: builtin_display_can_use_ioreg_nits_fallback
    with patch.object(
        display_brightness,
        "_current_display_context",
    ...
    # --- scenario: unsupported_external_display_does_not_borrow_builtin_fallback
    with patch.object(
        display_brightness,
        "_current_display_context",
...
def test_inactive_or_sleeping_display_skips_brightness_readers(monkeypatch) -> None:
    for active, asleep in ((False, False), (True, True)):
...
def test_main_display_identifier_is_resolved_for_every_read__and_2_more() -> None:
    # --- scenario: main_display_identifier_is_resolved_for_every_read
    with patch.object(
        display_brightness,
        "_current_display_context",
    ...
    # --- scenario: ioreg_nits_parser_uses_only_truthful_value_over_max
    ...
    # --- scenario: ioreg_query_is_bounded_and_uses_apple_arm_backlight
...
def test_display_services_rejects_failed_or_invalid_readings__and_2_more() -> None:
    # --- scenario: display_services_rejects_failed_or_invalid_readings
    for status, value in ((1, 0.5), (0, float("nan")), (0, float("inf")), (0, -0.1), (0, 1.1)):
    ...
    # --- scenario: display_services_returns_valid_fraction
    class _DisplayServices:
        @staticmethod
    ...
    # --- scenario: no_truthful_source_raises_instead_of_assuming_full_brightness
    with patch.object(
        display_brightness,
        "_current_display_context",
