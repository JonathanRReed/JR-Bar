...
def test_acknowledge_is_exact_idempotent_reversible_and_does_not_mutate_truth__and_2_more() -> None:
    # --- scenario: acknowledge_is_exact_idempotent_reversible_and_does_not_mutate_truth
    request = _request()
    original = request
    ...
    # --- scenario: resume_removes_only_the_selected_exact_request_key
    ...
    # --- scenario: same_request_id_under_two_source_instances_never_collides
    first = _request(
...
def test_forged_acknowledge_rechecks_current_phase_and_eligibility__and_2_more() -> None:
    # --- scenario: forged_acknowledge_rechecks_current_phase_and_eligibility
    for phase, eligibility in (
    ...
    # --- scenario: nonfinite_or_invalid_mutation_time_is_rejected
    for now in (float("nan"), float("inf"), -1.0, True, "now"):
    ...
    # --- scenario: cap_refuses_a_five_hundred_thirteenth_exact_acknowledgement
...
def test_reconcile_preserves_live_and_stale_hold_then_prunes_only_terminal_truth__and_2_more() -> None:
    # --- scenario: reconcile_preserves_live_and_stale_hold_then_prunes_only_terminal_truth
    ...
    # --- scenario: resume_with_a_different_current_key_cannot_remove_existing_acknowledgement
    original = _request()
    ...
    # --- scenario: controller_refresh_prunes_terminal_acknowledgements_and_saves
