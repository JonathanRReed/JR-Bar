...
def test_idle_screensaver_settings_default_off_clamp_and_decode(
    tmp_path: Path,
) -> None:
    defaults = AgentMonitorSettings()
    assert defaults.idle_screensaver_enabled is False
    assert defaults.idle_screensaver_effect is None
    assert defaults.idle_screensaver_after_minutes == 20

    target = tmp_path / "settings.json"
    settings = (
        AgentMonitorSettings()
        .with_idle_screensaver_enabled(True)
        .with_idle_screensaver_effect("rainbow")
        .with_idle_screensaver_after_minutes(45)
    )
    save_settings(settings, target)
    loaded = load_settings(target)

    assert loaded.idle_screensaver_enabled is True
    assert loaded.idle_screensaver_effect == "rainbow"
    assert loaded.idle_screensaver_after_minutes == 45
    payload = json.loads(target.read_text())
    assert payload["idle_screensaver_effect"] == "rainbow"
    assert payload["idle_screensaver_after_minutes"] == 45

    # The delay clamps into 5-1440 at both ends.
    low = AgentMonitorSettings().with_idle_screensaver_after_minutes(1)
    high = AgentMonitorSettings().with_idle_screensaver_after_minutes(99999)
    assert low.idle_screensaver_after_minutes == 5
    assert high.idle_screensaver_after_minutes == 1440

    # A blank or mistyped effect reads as "nothing picked", not an error.
    blank = AgentMonitorSettings().with_idle_screensaver_effect("   ")
    mistyped = AgentMonitorSettings().with_idle_screensaver_effect(12)
    assert blank.idle_screensaver_effect is None
    assert mistyped.idle_screensaver_effect is None

    target.write_text(
        json.dumps(
            {
                "settings_schema_version": 2,
                "idle_screensaver_enabled": "yes",
                "idle_screensaver_effect": 7,
                "idle_screensaver_after_minutes": "soon",
            }
        )
    )
    tolerant = load_settings(target)
    assert tolerant.idle_screensaver_enabled is False
    assert tolerant.idle_screensaver_effect is None
    assert tolerant.idle_screensaver_after_minutes == 20

    # A JSON bool is never a delay, even though True == 1 in Python.
    target.write_text(
        json.dumps(
            {
                "settings_schema_version": 2,
                "idle_screensaver_after_minutes": True,
            }
        )
    )
    boolean_delay = load_settings(target)
    assert boolean_delay.idle_screensaver_after_minutes == 20


def test_device_resting_glow_persists_and_defaults_to_zero(tmp_path: Path) -> None:
    """apply_calibration writes devices.N.resting_glow; to_dict must emit
    it or an applied glow silently reverts on the next load."""
    target = tmp_path / "settings.json"
    configured = AgentMonitorSettings().with_device_resting_glow("strip-a", 0.3)

    assert configured.devices[0].to_dict()["resting_glow"] == 0.3

    save_settings(configured, target)
    payload = json.loads(target.read_text(encoding="utf-8"))
    assert payload["devices"][0]["resting_glow"] == 0.3
    assert load_settings(target).resting_glow_for_device("strip-a") == 0.3

    # A row written before the field existed still loads at the 0.0 default.
    legacy = tmp_path / "legacy-settings.json"
    legacy.write_text(
        json.dumps({"devices": [{"id": "strip-b", "name": "Strip B", "path": "/dev/cu.strip-b"}]}),
        encoding="utf-8",
    )
    assert load_settings(legacy).resting_glow_for_device("strip-b") == 0.0
